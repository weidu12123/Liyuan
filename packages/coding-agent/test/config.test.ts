...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@liyuan/agent-runtime")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @liyuan/agent-runtime",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@liyuan/agent-runtime")).toBeUndefined();
		expect(getUpdateInstruction("@liyuan/agent-runtime")).toBe(
			"Update @liyuan/agent-runtime using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@liyuan/agent-runtime");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@liyuan/agent-runtime",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @liyuan/agent-runtime`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@liyuan/agent-runtime", undefined, {
			packageName: "@liyuan/agent-runtime",
			installSpec: "@liyuan/agent-runtime@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@liyuan/agent-runtime@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @liyuan/agent-runtime@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@liyuan/agent-runtime", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @liyuan/agent-runtime`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@liyuan/agent-runtime")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @liyuan/agent-runtime",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@liyuan/agent-runtime"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @liyuan/agent-runtime",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@liyuan/agent-runtime";
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@liyuan/agent-runtime")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@liyuan/agent-runtime")).toContain(
