...
```bash
npm install @liyuan/ai
```

TypeBox exports are re-exported from `@liyuan/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@liyuan/ai';
import { builtinModels } from '@liyuan/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@liyuan/ai/providers/anthropic';
import { openaiProvider } from '@liyuan/ai/providers/openai';
import { openrouterProvider } from '@liyuan/ai/providers/openrouter';
import { amazonBedrockProvider } from '@liyuan/ai/providers/amazon-bedrock';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@liyuan/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@liyuan/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@liyuan/ai';
...
```typescript
import { validateToolCall, type Tool } from '@liyuan/ai';
...
```typescript
import { builtinImagesModels } from '@liyuan/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@liyuan/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@liyuan/ai';
import { openAICompletionsApi } from '@liyuan/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@liyuan/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@liyuan/ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@liyuan/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@liyuan/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@liyuan/ai';
import { anthropicProvider } from '@liyuan/ai/providers/anthropic';
import { openaiProvider } from '@liyuan/ai/providers/openai';
import { googleProvider } from '@liyuan/ai/providers/google';
...
```typescript
import { createModels } from '@liyuan/ai';
import { anthropicProvider } from '@liyuan/ai/providers/anthropic';
...
- `@liyuan/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@liyuan/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@liyuan/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@liyuan/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@liyuan/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@liyuan/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@liyuan/ai/bedrock-provider';
...
```bash
npx @liyuan/ai login              # interactive provider selection
npx @liyuan/ai login anthropic    # login to specific provider
npx @liyuan/ai list               # list available providers
```
...
The legacy flow functions remain available via the `@liyuan/ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@liyuan/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@liyuan/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@liyuan/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@liyuan/ai`.
