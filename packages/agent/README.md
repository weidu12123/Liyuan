...
Stateful agent with tool execution and event streaming. Built on `@liyuan/ai`.

## Installation

```bash
npm install @liyuan/agent-core
```
...
```typescript
declare module "@liyuan/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@liyuan/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@liyuan/agent-core";
