...
import { parseCardFromSessionHead, summarizeToolResult, toWireHistory, toWireMsg } from "../server/wire.ts";
...
test("assistant 正文 → narrative 通道；toolCall 丢弃、thinking 提取为折叠字段、正文原样", () => {
	const msg = {
		role: "assistant",
		content: [
			{ type: "thinking", thinking: "内心活动" },
			{ type: "text", text: "*她抬头。*\n\n\"你醒了。\"" },
			{ type: "toolCall", id: "t1", name: "lorebook_search", arguments: {} },
		],
	};
