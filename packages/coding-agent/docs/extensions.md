...
| Package | Purpose |
|---------|---------|
| `@liyuan/agent-runtime` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@liyuan/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@liyuan/tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@liyuan/agent-runtime";
...
```typescript
import { isBashToolResult } from "@liyuan/agent-runtime";
...
```typescript
import { createLocalBashOperations } from "@liyuan/agent-runtime";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@liyuan/agent-runtime";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@liyuan/ai";
...
```typescript
import type { AutocompleteItem } from "@liyuan/tui";
...
```typescript
import { withFileMutationQueue } from "@liyuan/agent-runtime";
...
**Important:** Use `StringEnum` from `@liyuan/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@liyuan/agent-runtime";
...
```typescript
import { createBashTool } from "@liyuan/agent-runtime";
...
```typescript
import { keyHint } from "@liyuan/agent-runtime";
...
```typescript
import { Text, Component } from "@liyuan/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@liyuan/agent-runtime";
import { matchesKey } from "@liyuan/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@liyuan/agent-runtime";
